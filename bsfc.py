...
# Load the data
data = load_data()
filtered_data = data[["Engine Load(%)", "Engine RPM(rpm)", "Fuel used (inst)"]].apply(pd.to_numeric, errors='coerce')
...
filtered_data["Brake Power (kW)"] = (filtered_data["Engine Load(%)"] * filtered_data["Engine RPM(rpm)"]) / 1000
...
# Now, calculate BSFC
filtered_data["BSFC (g/kWh)"] = filtered_data["Fuel used (grams)"] / filtered_data["Brake Power (kW)"]

# Display the result
print(
    filtered_data[
        ["Engine RPM(rpm)", "Engine Load(%)", "Fuel used (inst)", "Brake Power (kW)", "BSFC (g/kWh)"]
    ].head()
)
...
filtered_data = filtered_data.groupby(
    ...
    ["Engine RPM(rpm)", "Engine Load(%)", "Fuel used (inst)", "BSFC (g/kWh)"]
].reset_index()
